    ...
    .then(async (statusResponse) => {
      ...
      if (transactionStatus == 'capture') {
        if (fraudStatus == 'accept') {
          ...
          const queries = {
            where: { code: orderId },
            data: { status: 4, payment_date: new Date(statusResponse.transaction_time).getTime() }
          }
        ...
        const queries = {
          where: { code: orderId },
          data: { status: 3, expired_date: new Date(statusResponse.transaction_time).getTime() }
        }
        const sale = await prisma.sale.update(queries);
        reply.code(200).send({ message: summary });
